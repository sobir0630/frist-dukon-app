...
def load_data():
    ...
    try:
        ...
        deleted_file = "database/deleted_phones.json"
...
def delete_phone():
    ...
    def confirm_delete():
        ...
        if confirm:
            ...
            try:
                with open("database/deleted_phones.json", "w", encoding="utf-8") as f:
                    json.dump(deleted_phones, f, ensure_ascii=False, indent=4)
            except Exception as e:
                print(f"Xatolik: {e}")

            with open("telefon_data.json", "r") as json_file:
...
#             # Ekranni to‘liq holatda ochish (faqat oynani maksimal qilish)
#             self.root.geometry(f"{self.root.winfo_screenwidth()}x{self.root.winfo_screenheight()}+0+0")

#             # ESC tugmasi bilan dasturni yopish
#             self.root.bind("<Escape>", lambda e: self.root.destroy())


            
#             # JSON fayl yo'li
#             self.data_file = "database/notebook_data.json"
#             self.load_data()
...
#                 messagebox.showinfo("saqlandi", "saqlandi")
#             except Exception as e:
...
#         def choose_color(self, alarms_window):
#             """Rangni tanlash"""
#             color = colorchooser.askcolor()[1]
#             if color:
#                 self.text_area.config(fg=color)
#                 self.status_label.configure(text=f"🎨 Rang o'zgartirildi: {color}")
#                 alarms_window.transient(self.root)
#                 alarms_window.grab_set()
#                 alarms_window.focus_set()
...
#             files_window.transient(self.root)      # Asosiy oynaga bog'lash
#             files_window.grab_set()                # Modal qilish
#             files_window.focus_set()               # Fokus berish
...
#                 for _ in range(10):  # 5 marta tovush chiqarish
#                     winsound.Beep(1500, 1000)  # 1000Hz, 500ms
...
# #     # Dasturni ishga tushirish
# # root = open_eslatma_file(NotebookApp)
# # open_eslatma_file()

    # NotebookApp()

def open_eslatma_file():
    """Eslatma faylini ochish"""
    ctk.set_appearance_mode("dark")
    ctk.set_default_color_theme("blue")
    ...
    class NotebookApp:
                ...
                def check_alarms():
                    while True:
                        try:
                            ...
                            for alarm in self.data["alarms"]:
                                if alarm["active"] and alarm["time"] == current_time:
                                    alarm["active"] = False  # Budilnikni faqat 1 marta ishlatish
                                    self.save_data()
                                    self.root.after(0, lambda a=alarm: self.show_alarm_message(a))
                                    threading.Thread(target=self.play_alarm_sound, daemon=True).start()
                            time_module.sleep(60)  # Har daqiqada tekshiradi
                        except Exception as e:
                            print("Xatolik:", e)
        ...
        def play_alarm_sound(self):
            """Budilnik tovushini chiqarish (bloklamasligi kerak)"""
            try:
                for _ in range(5):
                    winsound.Beep(1500, 1000)  # 1000ms tovush
                    time_module.sleep(0.3)
            except:
                print("🔔 BUDILNIK! 🔔")
        ...
        """TODO  shu yerda katta xatolik bor dastur kirganda kodni
                oshlatmayabdi ochmayabdi yani TODO"""  


    # Klassni chaqiramiz — bo'lmasa oyna ochilmaydi
    NotebookApp()
...
def view_all_history():
    ...
    try:
        ...
        deleted_phones = []
        if os.path.exists("database/deleted_phones.json"):
            try:
                with open("database/deleted_phones.json", "r", encoding="utf-8") as f:
                    deleted_phones = json.load(f)
            except Exception:
                deleted_phones = []
